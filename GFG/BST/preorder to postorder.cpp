...
int cnt;
void preorder(Node* head, int arr[],int st,int en, int n){
    if(cnt==n){
        return;
    }
    Node* temp= newNode(arr[cnt]);
    if(temp->data > st && temp->data < head->data){
        head->left=temp;
        cnt++;
        preorder(head->left,arr,st,head->data,n);
        if(cnt<n)
            Node* temp=newNode(arr[cnt]);
    }
    if(cnt< n && temp->data > head->data && temp->data < en){
        head->right = temp;
        cnt++;
        preorder(head->right,arr,head->data,en,n);
    }
}
...
Node* constructTree(int pre[], int size)
{
    ...
    cnt=1;
    preorder(head,pre,INT_MIN,INT_MAX,size);
    return head;
}
